...
class User
{
    ...
    public function register(Request $request){
        ...
        if($dbnum==null){
                ...
                $dbdata = ['id'=>'','openid' =>$openid,'channel' => $channel,'scene' => $scene,'tribute' => 0,'create_time' =>$time ,'updata_time' =>$time];
                $userId= db('user')->insertGetId($dbdata);//返回自增ID
                $userdata=['id'=>$userId,'openid' =>$openid,'channel' => $channel,'scene' => $scene,'tribute' => 0,'create_time' =>$time ,'updata_time' =>$time];
